...
file_lst = ['/home/haominwe/code/D-Transformer/',
            ...
            '/ocean/projects/cis220071p/hwen3/code/D-Transformer/']
...
from algorithm.dataset_mask_one import TransformerDataset, PretrainPadder
from algorithm.transfomer_uc.model import VanillaTransformer, save2file
...
def test_model_with_loss(model, test_loader, device, params, save2file, mode):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            # if i== 0: # used to test whether the val data at each epoch is the same
            #     print('val \n',input_seq[...,0][2])
            # i += 1
...
def main(params):
    params['model'] = 'Transformer AU EU'
    params['model_save2file'] = (VanillaTransformer, save2file)
    ...
    return run(params, TransformerDataset, process_batch, test_model_with_loss, collate_fn=(train_collate_fn, val_collate_fn, test_collate_fn))
...
def get_params():
    ...
    # Model parameters
    parser.add_argument('--model', type=str, default='vanilla Transformer')
    ...
    parser.add_argument('--dropout', type = float, default=0.5) # for epistemic uncertainty, using mc dropout

    # parser.add_argument('--dow_mode', type=str, default='use_dow') # to set different types of pos,  choice: no_use, use_poi,
...
if __name__ == '__main__':
    ...
    # for test the code
    params['dataset'] = '7_22_trial2_test'
    ...
    params['early_stop_start_epoch'] = 5
