...
class VanillaTransformer(nn.Module):
    ...
    def __init__(self, args: dict):
        ...
        self.emb_con = nn.Linear(self.d_c, self.d_h)
        ...
        for f in self.dis_feature_list:
            class_num = f['class_num']
            # dec_f = nn.Sequential(nn.Linear(self.d_h, class_num), nn.Dropout(self.dropout))
            dec_f = nn.Linear(self.d_h, class_num)

            self.dec_dis.append(dec_f)

            # self.dec_dis_sigma.append(nn.Sequential(nn.Linear(self.d_h, class_num), nn.Dropout(self.dropout)))
        ...
        self.encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=self.d_h, nhead=self.num_head, batch_first=True, dropout=self.dropout),
    ...
    def forward(self, input_seq, position, u, return_raw = True):
        ...
        params:
          input_seq (torch.FloatTensor): contains the input feature
             (B, L, d_x),  B is the batch size, L is the sequence length, d_x is the feature size in one tuple
        ...
        src_key_padding_mask = token[..., 0] == PAD_TOKEN

        # embed continous features
        con_x = input_x[:, :, self.con_feature_idx]
        con_e = self.emb_con(con_x) #(B, L, d_h)
        ...
        pos_e = self.pos_encode_layer(seq_pos)

        # init hidden state
        h = con_e

        # add discrete feature
        for f_e in dis_e:
            h += f_e
        ...
        h = self.encoder(h, src_key_padding_mask=src_key_padding_mask) # (B, L, d_h) --> (B, L, d_h)
    ...
    def sample(self, input_seq, position, u, return_raw = True, n_sample = 5):
        ...
        # return self.forward(input_seq_tmp, position_tmp, u_tmp, return_raw)
    ...
    def model_file_name(self):
        ...
        file_name = f'vanilla_dis_transformer_uc_-{file_name}'
        return file_name
...
def save2file(params):
    file_name = ws + f'/output/metrics/tr_au_eu.csv'
    head = [
        # data setting
        'dataset', 'seed',
        # model parameters
