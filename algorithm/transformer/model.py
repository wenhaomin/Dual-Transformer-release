...
class VanillaTransformer(nn.Module):
    ...
    def __init__(self, args: dict):
        ...
        self.encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=self.d_h, nhead=self.num_head, batch_first=True), num_layers=self.num_layer)
...
def save2file(params):
    file_name = ws + f'/output/metrics/7_24_vanilla_transformer.csv'
    head = [
        # data setting
        'dataset', 'seed',
        # model parameters
        'model', 'd_h', 'num_head', 'num_layer', 'user_mode', 'pos_mode', 'mask_ratio',
        # evalution setting
