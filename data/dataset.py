...
class EventDataset(object):
    ...
    def get_dataset(self):
        ...
        if self.is_test:
            df = pd.read_csv(self.params['fin'] + "/stay.csv", sep=',', encoding='utf-8', nrows=10000)
        else:
            df = pd.read_csv(self.params['fin'] + "/stay.csv", sep=',', encoding='utf-8')

        df = df.rename(columns={'stay_time': 'stay_duration'})

        # filter users
        u_lst = df['uid'].unique()
        u_lst = u_lst[: 20000] # control the user number
        df = df[df['uid'].isin(u_lst)]


        # convert the gps to x,y point (unit: km) centerd at the middle point
        if 'x' not in df.columns.tolist():
            df['x'], df['y'] = gps_to_normed_xy(df['lng'].tolist(), df['lat'].tolist(), region='LA', scale=1000) # region: LA means Los Angles, scale 1000, means x, y unit is 1 km.
        ...
        if not os.path.exists(self.stay_stat_path):
            df.describe(percentiles=[0.25, 0.5, 0.75, 0.95]).to_csv(self.stay_stat_path)
        ...
        if self.is_test == False:
            if self.mode == 'train':
                ...
                df.describe(percentiles=[0.25, 0.5, 0.75, 0.95]).to_csv(self.stay_train_stat_path) # save the statistics information in train
            elif self.mode == 'val':
...
def get_params():
    ...
    # dataset parameters
    parser.add_argument('--fin',  type=str, default=ws + '/data/raw/trial2/')
    parser.add_argument('--day_window', type=int, default=2)
    parser.add_argument('--data_name', type=str, default='7_22_trial2_2week')
    # is_test = True if platform.system() == 'Windows' else False
    is_test = False
    parser.add_argument('--is_test', type=bool, default=is_test)
    num_thread = 4 if platform.system() == 'Windows' else 40
...
def main():
    ...
    for mode in [ 'train', 'val', 'test']: #'test', 'train', 'val',
        if mode == 'train':
            ...
            params['fout'] =  fout
            week = 2
            params['train_ratio'] = get_after_dates("2024-01-01", list(range(7 * week)))
            ...
            params['fout'] = fout
            week = 1
            params['val_ratio'] = get_after_dates("2024-01-22", list(range(7 * week)))
            EventDataset(params).get_dataset()
            ...
            params['fout'] = fout
            week = 2
            params['test_ratio'] = get_after_dates("2024-01-29", list(range(7 * week)))
            EventDataset(params).get_dataset()
...
if __name__ == '__main__':
    ...
    if 0: # get dates for dataset
        # df = pd.read_csv(ws + '/data/raw/trial2/stay.csv')
        # dates = sorted(df['dates'])
        # print('Total number of dates:', len(dates))

        # for train
        week = 3
        dates =  get_after_dates("2024-01-01", list(range(7 * week)))
        print(dates)
        print(get_day_of_week(dates[-1]))

        # for val
        week = 1
        dates = get_after_dates("2024-01-22", list(range(7 * week)))
        print(dates)
        print(get_day_of_week(dates[-1]))

        # for test
        week = 2
        dates = get_after_dates("2024-01-29", list(range(7 * week)))
        print(dates)
        print(get_day_of_week(dates[-1]))


    if 0:# test for get_dateset_stat
        data_name = 'trial2'
        data_file =  ws + f'/data/dataset/{data_name}/'
        for mode  in ['train', 'val', 'test']:
            fin = data_file + f'{mode}.npy'
            fout = data_file + 'dataset_describle.csv'
            data = np.load(fin, allow_pickle=True).item()
            get_dateset_stat(data, mode, fout)

    if 0: # test for gps to xy
        from utils.util import gps2xy
        ...
        df = pd.read_csv(ws + '/data/raw/trial2/' + "/stay.csv", sep=',', encoding='utf-8', nrows=10000)
        x, y = gps_to_normed_xy(df['lng'], df['lat'], region='LA')
        print(x,y)
