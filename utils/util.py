...
def train_val_test(train_loader, val_loader, test_loader, model, device, process_batch, test_model, params, save2file):
    ...
    import platform
    # file = '/data/shurui/haomin/code/D-Transformer/' if platform.system() == 'Linux' else ws  # '/data/shurui/D-Transformer/'

    # to run on different platform
    # file_dict = {
    #     'Linux-5.4.0-139-generic-x86_64-with-glibc2.31': '/data/shurui/haomin/code/D-Transformer/', # for la3
    #     'Linux-4.18.0-477.27.1.el8_8.x86_64-x86_64-with-glibc2.28': ws, # for psc
    # }
    # file = file_dict.get(platform.platform(), ws)
    # forecast_path = file + '/output/forecast/' + trial_name + '.pkl' # not enough space in server, save it in other space
    ...
    for epoch in range(params['num_epoch']):
        ...
        with tqdm(train_loader, total=len(train_loader), postfix=postfix) as t:
            ...
            for i, batch in enumerate(t):
                ...
                # if i == 0: #test whether the data each epoch is the same
        ...
        # for debug:
        # model_path_epoch = ws + f'/output/model_debug/{epoch}.pkl'
        # model_path_epoch = dir_check(model_path_epoch)
        ...
        # print('\nval result:', f"current loss:{round(val_epoch_log['total'], 4)}",  val_result.to_str(), f'Best {early_stop_metric}:', round(early_stop.best_metric(),3), '| Best epoch:', early_stop.best_epoch)

        # for debug:
        # lst = ['start_time_minute-mae', 'stay_duration-mae', 'poi-acc']
        # def to_str(result_dict: dict):
        #     s = ''
        #     for k in lst:
        #         temp = f'{k}: {result_dict[k]} | '
        #         s += temp
        #     return s


        # params['best_epoch'] = -1
        # params['best_metric'] = -1
        # test_result = test_model(model, test_loader, device, params, save2file, 'test')
        # print('\ntest result:',  to_str(test_result.to_dict()))
        # print('\nval result:', to_str(val_result.to_dict()))
        # print('--' * 50)
